...
public class HarvController{
	...
	private void findRotationAcceleration(int iterationTime){//code for calibrating robot
	...
	private void agmentedDriveControl(){
		final double rpm = 0;
		final double rotationCoefficiant = 8;
		final double skewTolerance = 4;
		final double skewToleranceCoefficiant = 1.00;
		input.update();
		...
		degreesRotated += (magRot * rotationCoefficiant);
		double skew = Math.abs(degreesRotated - sensors.getAngle());
		
		if(degreesRotated > sensors.getAngle()*skewToleranceCoefficiant - skewTolerance)	magRot = magRot - (-skew/30);
		else if (degreesRotated < sensors.getAngle()*skewToleranceCoefficiant + skewTolerance) magRot = magRot -(skew/30);
		else;
	}
